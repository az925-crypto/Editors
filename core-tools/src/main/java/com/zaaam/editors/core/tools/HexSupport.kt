...
// PUBLIC sejak UI Phase 2: konstanta + formatter dipakai HexScreen di :app.
const val MAX_HEX_FILE_BYTES: Long = 16L * 1024 * 1024

// Stack undo byte-edit (index, nilaiLama) — cap mencegah memori membengkak.
const val HEX_UNDO_MAX = 32

const val HEX_ROW_BYTES = 16
...
fun toAscii(b: Byte): Char? {
    val v = b.toInt() and 0xFF
    return if (v in 0x20..0x7E) v.toChar() else null
}

// Kontrak UI (perf-reviewer): panggil HANYA per baris visible LazyColumn + remember keyed
// (versi bytes, rowStart). DILARANG precompute seluruh file — 1M baris > 100MB heap.
fun formatRow(bytes: ByteArray, rowStart: Int): List<ByteCell> {
